...
class SpellAssigner extends Component {
  ...
  filteredSpellsInClass = (inClass) => this.props.spells.filter((spell) => {
    let include = true;
    if (inClass) {
              ...
              <td style={{ verticalAlign: 'top' }}>
                ...
                <ul>
                  {this.filteredSpellsInClass(false).map((spell) => (
                    <li
                      key={spell.id}
                      onClick={self.handleAssignSpell(spell)}
                      onKeyDown={self.handleAssignSpell(spell)}
                    >
                      {spell.name}
                    </li>
                  ))}
              ...
              <td style={{ verticalAlign: 'top' }}>
                ...
                <ul>
                  {this.filteredSpellsInClass(true).map((spell) => (
                    <li
                      key={spell.id}
                      onClick={self.handleUnassignSpell(spell)}
                      onKeyDown={self.handleUnassignSpell(spell)}
                    >
                      {spell.name}
                    </li>
                  ))}
